...
RRAMEN [^1] is an interactive tool that supports city-scale mobility-related queries by different types of users with different needs. It is based on the concept of Relative Reachability (RR), 
...
[^1]: The bowl icon used in our demo was obtained from: https://www.fourjay.org/maxpic/ioiiwbw/.
...
Here we envision a scenario where a planner would select a number of representative points in the city, e.g., shopping malls, and identify their RR w.r.t. a region, e.g., downtown. In this case, as illustrated below, the colors of each destination point reflect how much more likely it is for a transportation mode to be the most efficient w.r.t. the source locations. That is, the closer to red (or blue) the more likely it is, over the set of source locations, that private cars (or public transit) reach that point earlier.
...
Finally, RRAMEN also allows planners to remove/add road segments and analyze the impact of those changes. In order to do so, users must right click the map on the desired road segment. A popup box will be displayed to confirm the user's selection, as shown below (left side figure). The selected road segments will be removed from the road network and be displayed on the sidebar menu. A removed segment can be added back to the network by checking its corresponding checkbox on the sidebar menu.
