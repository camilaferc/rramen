...
class LoadTransportationNetwork:
    ...
    def loadNodes(self):
        ...
        try:
            ...
            while row is not None:
                (node_id, lat, lon, type, stop_id, route_id) = row
                #print(node_id, lat, lon, type, stop_id, route_id)
                self.graph.addNode(node_id, lat, lon, type, stop_id, route_id)
                row = cursor.fetchone()
    ...
    def loadEdges(self):
        ...
        try:
            ...
            while row is not None:
                (source, target, type, modes, t_monday, t_tuesday, t_wednesday, t_thursday, t_friday, t_saturday, t_sunday) = row
                ...
                self.graph.addEdge(source, target, type, modes, {MultimodalNetwork.PUBLIC:TimeTable(timetable)})
                #print(node_id, lat, lon, type, stop_id, route_id)
                row = cursor.fetchone()
